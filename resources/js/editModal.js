        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (module == "menus") {
                    ...
                    modal
                        .find(`option[value=${data.parent_id}]`)
                        .prop("selected", true);
                    modal.find("input[name=is_parent]").val(data.parent_id);
                    modal
                        .find(".parent_id")
                        .prop("disabled", data.parent_id == 0);
                }
                ...
                if (module == "sliders") {
                    ...
                    modal
                        ...
                        .removeClass("hidden");
                    modal
                        .find("input[name=old-slider-file-id]")
                        .val(data.old_slider_file_id);
                ...
                if (module == "products" && !type) {
                    ...
                    modal
                        ...
                        .removeClass("hidden");
                    modal
                        .find("input[name=old-product-file-id]")
                        .val(data.old_product_file_id);
                    modal
                        .find(
                            `select#category_id option[value=${data.product_info.category_id}]`,
                        )
                        .prop("selected", true);
                    modal
                        .find(
                            `select#up_sales option[value=${data.product_info.up_sales}]`,
                        )
                        .prop("selected", true);
                    data.detail_imgs.forEach((element, index) => {
                        modal
                            .find(`img.product-subfile-${index + 1}-img`)
                        ...
                        modal
                            .find(
                                `input[name=old-product-subfile-${index + 1}-id]`,
                            )
                            .val(element.id);
                    });
                    ...
                    modal
                        .find(`option[value=${data.product_info.attribute_id}]`)
                        .prop("selected", true);

                    if (data.view_variants != "") {
                        modal
                            .find(".edit_attribute_variant_check")
                            .html(data.view_variants);
                    }
                ...
                if (module == "posts" && !type) {
                    ...
                    modal
                        ...
                        .removeClass("hidden");
                    modal
                        .find("input[name=old-post-file-id]")
                        .val(data.old_post_file_id);
                    modal
                        .find(`option[value=${data.post_info.category_id}]`)
                        .prop("selected", true);
                    modal.find(".fake-remove-file").removeClass("hidden");
                    if (data.post_info.content) {
                        tinymce
                            .get("edit-post-content")
                            .setContent(data.post_info.content);
                    }
                }

                // categories
                if (
                    (module == "posts" || module == "products") &&
                    type == "categories"
                ) {
                    ...
                    modal
                        .find("input[name=is_parent]")
                        .val(data.category_info.parent_id);
                    modal
                        .find(`option[value=${data.category_info.parent_id}]`)
                        .prop("selected", true);
                    modal
                        .find("select[name=parent_category]")
                        .prop(
                            "disabled",
                            data.category_info.parent_id == 0 ? true : "",
                        );
                    ...
                    if (data.img_url) {
                        modal
                            ...
                            .removeClass("hidden");
                        modal
                            .find("input[name=old-category-file-id]")
                            .val(data.old_category_file_id);
                        modal.find(".fake-remove-file").removeClass("hidden");
                    }
                }
                ...
                if (module == "roles") {
                    ...
                    data.permissions.forEach((permission_id) => {
                        modal
                            .find(`input[value=${permission_id}]`)
                            .prop("checked", true);
                    });
                }
            },
