<section>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-3">
        ...
        @method('put')
            ...
            <div>
                <x-input-field.field label="Mật khẩu hiện tại" type="password" name="current_password" id="update_password_current_password" required="*"/>
                <x-form-element.input-error :messages="$errors->updatePassword->get('current_password')" class="mt-1" />
            </div>
            
            <div>
                <x-input-field.field label="Mật khẩu mới" type="password" name="password" id="update_password_password" required="*"/>
                 <x-form-element.input-error :messages="$errors->updatePassword->get('password')" class="mt-1" />
            </div>

            <div>
                <x-input-field.field label="Xác nhận mật khẩu" type="password" name="password_confirmation" id="update_password_password_confirmation" required="*"/>
