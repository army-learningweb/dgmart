        ...
        <div class="border-t border-gray-500/20 parent_check_all">
            <div class="py-2" colspan="4">
                <div class="flex items-center gap-3">
                    <input type="checkbox" class="check_all_permission rounded-sm" id="check_all_permission_{{$group}}"> 
                    <label for="check_all_permission_{{$group}}" class="text-blue-600">{{ ucfirst($group) }}</label>
                </div>
            </div>
        </div>
            ...
            @foreach ($items as $item)
                <div class="py-3">
                    <div class="flex justify-between items-center gap-3">
                        ...
                        <label for="variant_{{$item->id}}" class="w-[150px] truncate">{{ $item->name }}</label>
                    </div>
                </div>
            @endforeach
        </div>
